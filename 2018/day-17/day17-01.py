...
def scan(xx, y):
    ...
    for x in range(xx, 0, -1):
        if (x, y) in D:
            ...
            left_overflow = False
            break
        if (x, y + 1) not in D and ((x, y + 1) not in water
                                    or water[(x, y + 1)] == "|"):
            leftx = x
            left_overflow = True
            break
...
print "part 1 (all) =", len([w for w in water if w[1] >= miny])
print "part 2 (flowing) =", len(
    [w for w in water if w[1] >= miny and water[w] == "~"])
